...
const getGenresList = async (type: string, genres: any, page: number) => {
    try {
        const result = await axios.get(`${defaultPath}/${type}/top_rated?api_key=${API_KEY}&language=${language}&page=${page}&with_genres=${genres}`);
        return result.data;
    } catch (err) {
        console.error(`Failed to fetch ${type} ${genres}`, err);
        throw err;
    }
}
